...
class PdoGsb
{
    ...
    /**
     * Fonction statique qui créer l'unique instance de la classe
    ...
    {
        $requetePrepare = PdoGsb::$monPdo->prepare('SELECT visiteur.id AS id, 
                    visiteur.nom AS nom, 
     ...
     * a une modification de la structure - transformation du champ date-
     *
    ...
    public function getLesFraisHorsForfait($idVisiteur, $mois)
    {
        ...
        for ($i = 0; $i < count($lesLignes); $i ++) {
            ...
            $instanceRetournerDate = new fonction();
            $retournerDate = $instanceRetournerDate->dateAnglaisVersFrancais($date);
            $lesLignes[$i]['date'] = $retournerDate;
        }
        return $lesLignes;
    }
    ...
    {
        $requetePrepare = PdoGsb::$monPdo->prepare('SELECT visiteur.id as id, visiteur.nom, visiteur.prenom FROM visiteur ');
        $requetePrepare->execute();
     ...
     *            tableau associatif de clée idFrais et
     ...
     *            ID de la ligne hors forfait concernée
     * @param String $libelle
     *            Libellé relevé du frais hors forfait
     * @param String $date
    ...
    {
        $requetePrepare = PdoGSB::$monPdo->prepare('UPDATE lignefraishorsforfait SET libelle = :unLibelle, date = :uneDate, mois = :unMois, montant = :unMontant WHERE id = :unIdFrais');
    ...
    {
        $instanceDate = new fonction();
        
        $dateFr = $instanceDate->dateFrancaisVersAnglais($date);
    ...
    {
        $requetePrepare = PdoGSB::$monPdo->prepare('SELECT fichefrais.mois AS mois FROM fichefrais ' . 'WHERE fichefrais.idvisiteur = :unIdVisiteur AND fichefrais.idetat = "VA" ' . 'ORDER BY fichefrais.mois desc');
        $requetePrepare->bindParam(':unIdVisiteur', $idVisiteur, PDO::PARAM_STR);
        $requetePrepare->execute();
        $lesMois = array();
        while ($laLigne = $requetePrepare->fetch()) {
    ...
    {
        $requetePrepare = PdoGSB::$monPdo->prepare('SELECT MAX(fichefrais.mois) AS mois FROM fichefrais ' . 'WHERE fichefrais.idvisiteur = :unIdVisiteur   AND fichefrais.idetat = "VA" ' . 'ORDER BY fichefrais.mois');
        $requetePrepare->bindParam(':unIdVisiteur', $idVisiteur, PDO::PARAM_STR);
        $requetePrepare->execute();
    ...
    {
        $requetePrepare = PdoGSB::$monPdo->prepare('SELECT visiteur.mdp FROM visiteur WHERE visiteur.id = :unIdVisiteur');
        $requetePrepare->bindParam(':unIdVisiteur', $idVisiteur, PDO::PARAM_STR);
        $requetePrepare->execute();
    ...
    {
        $requetePrepare = PdoGSB::$monPdo->prepare('UPDATE visiteur ' . 'SET mdp = :unMotDePasse ' . 'WHERE visiteur.id = :unIdVisiteur ');
        $requetePrepare->bindParam(':unIdVisiteur', $idVisiteur, PDO::PARAM_STR);
    ...
    {
        $requetePrepare = PdoGsb::$monPdo->prepare('SELECT visiteur.id FROM visiteur WHERE visiteur.login = :unLogin');
        $requetePrepare->bindParam(':unLogin', $login, PDO::PARAM_STR);
        $requetePrepare->execute();
     ...
     *
     *
     * @return les informations du visiteur ayant l'id le plus petit sous la forme d'un tableau associatif
     */
    public function getVisiteurMin()
    {
        $requetePrepare = PdoGsb::$monPdo->prepare('SELECT visiteur.nom as nom, visiteur.prenom as prenom, ' . 'visiteur.id as id FROM visiteur ORDER BY id ASC LIMIT 1');
        $requetePrepare->execute();
        return $requetePrepare->fetch();
    }
